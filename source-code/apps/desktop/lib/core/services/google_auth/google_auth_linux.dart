...
/// Linux Google OAuth.
///
/// The official `google_sign_in` plugin has no Linux implementation, so we
/// run Google's "loopback IP address" desktop OAuth flow with PKCE:
...
class LinuxGoogleAuth implements GoogleAuth {
  LinuxGoogleAuth({
  ...
  /// Fixed loopback port shared with the macOS / Windows flows so a single
  ...
  static const _persistedFileName = 'linux_google_auth.json';

  _LinuxGoogleCredentials? _currentUser;
  ...
  Future<GoogleCredentials?> signIn() async {
    ...
    try {
      ...
      debugPrint(
        '[LinuxGoogleAuth] starting Google OAuth — redirect_uri=$redirectUri',
      );
      ...
      final creds = _LinuxGoogleCredentials._(
        owner: this,
        email: email,
  ...
  Future<void> signOut() async {
    ...
    if (token != null && token.isNotEmpty) {
      try {
        ...
        debugPrint('[LinuxGoogleAuth] revoke failed: $e');
      }
    }
    _currentUser = null;
  ...
  Future<_LinuxGoogleCredentials?> _refreshAccessToken(
      _LinuxGoogleCredentials creds) async {
    ...
    try {
      ...
      if (res.statusCode != 200) {
        debugPrint(
            '[LinuxGoogleAuth] refresh failed (${res.statusCode}): ${res.body}');
        _currentUser = null;
        await _clearPersistedFile();
        return null;
      }
      ...
      debugPrint('[LinuxGoogleAuth] refresh error: $e');
      return null;
    }
  }
  ...
  Future<String> _fetchEmail(String accessToken) async {
    try {
      ...
      debugPrint('[LinuxGoogleAuth] userinfo failed: $e');
    }
    return 'Connected';
  }
  ...
  Future<void> _persist() async {
    ...
    try {
      ...
      debugPrint('[LinuxGoogleAuth] persist failed: $e');
    }
  }
  ...
  Future<void> _hydrateFromDisk() async {
    ...
    try {
      ...
      _currentUser = _LinuxGoogleCredentials._(
        owner: this,
        email: email,
        accessToken: accessToken,
        refreshToken: refreshToken,
      ...
      debugPrint('[LinuxGoogleAuth] hydrate failed: $e');
    }
  }
...
class _LinuxGoogleCredentials implements GoogleCredentials {
  _LinuxGoogleCredentials._({
    required LinuxGoogleAuth owner,
    required this.email,
