...
///
/// The official `google_sign_in` plugin has no Windows implementation, so we
...
class WindowsGoogleAuth implements GoogleAuth {
  WindowsGoogleAuth({
  ...
  /// Fixed loopback port shared with the macOS / Linux flows so a single
  ...
  Future<GoogleCredentials?> signIn() async {
    ...
    try {
      ...
      debugPrint(
        '[WindowsGoogleAuth] starting Google OAuth — redirect_uri=$redirectUri',
      );
      ...
      final creds = _WindowsGoogleCredentials._(
        owner: this,
        email: email,
  ...
  Future<void> signOut() async {
    ...
    if (token != null && token.isNotEmpty) {
      try {
        ...
        debugPrint('[WindowsGoogleAuth] revoke failed: $e');
      }
    }
    _currentUser = null;
  ...
  Future<_WindowsGoogleCredentials?> _refreshAccessToken(
      _WindowsGoogleCredentials creds) async {
  ...
  Future<String> _fetchEmail(String accessToken) async {
    try {
      ...
      debugPrint('[WindowsGoogleAuth] userinfo failed: $e');
    }
    return 'Connected';
  }
  ...
  Future<void> _persist() async {
    ...
    try {
      ...
      debugPrint('[WindowsGoogleAuth] persist failed: $e');
    }
  }
  ...
  Future<void> _hydrateFromDisk() async {
    ...
    try {
      ...
      _currentUser = _WindowsGoogleCredentials._(
        owner: this,
        email: email,
        accessToken: accessToken,
        refreshToken: refreshToken,
      ...
      debugPrint('[WindowsGoogleAuth] hydrate failed: $e');
    }
  }
...
class _WindowsGoogleCredentials implements GoogleCredentials {
  _WindowsGoogleCredentials._({
    required WindowsGoogleAuth owner,
    required this.email,
  ...
  final WindowsGoogleAuth _owner;
  @override
  final String email;
  String _accessToken;
