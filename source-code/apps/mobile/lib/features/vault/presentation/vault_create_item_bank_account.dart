...
enum _BankAccountSectionKind { primary, branch }

const List<String> _bankAddMoreOptions = <String>[
  'custom field',
  'Sensitive Text',
  'note',
];

class AddBankAccountItemModal extends ConsumerStatefulWidget {
  const AddBankAccountItemModal({
    super.key,
  ...
  @override
  ConsumerState<AddBankAccountItemModal> createState() =>
      _AddBankAccountItemModalState();
}

class _AddBankAccountItemModalState
    extends ConsumerState<AddBankAccountItemModal> {
  late final TextEditingController _titleController;
  late final TextEditingController _notesController;
  late final TextEditingController _tagController;
  late final List<CreditCardFieldDraft> _primaryFields;
  late final List<CreditCardFieldDraft> _branchFields;
  ...
  void initState() {
    ...
    _primaryFields = <CreditCardFieldDraft>[
      ...
      CreditCardFieldDraft(label: 'type', valueHint: 'type', removable: true),
      CreditCardFieldDraft(
      ...
      CreditCardFieldDraft(label: 'SWIFT', valueHint: 'text', removable: true),
      CreditCardFieldDraft(label: 'IBAN', valueHint: 'text', removable: true),
      CreditCardFieldDraft(
        label: 'PIN',
        valueHint: 'password',
        isSecret: true,
    ...
    _branchFields = <CreditCardFieldDraft>[
      CreditCardFieldDraft(label: 'phone', valueHint: 'text', removable: true),
      CreditCardFieldDraft(
        label: 'address',
        valueHint: 'text',
        removable: true,
    ...
    if (edit != null) {
      ...
      // Pre-fill section fields from the entry.
      final allDrafts = <CreditCardFieldDraft>[
        ..._primaryFields,
        ..._branchFields,
      ];
      for (final draft in allDrafts) {
        final label = draft.labelController.text.trim().toLowerCase();
        final storageKey = bankStorageKeyForLabel(label).toLowerCase();
        for (final field in edit.fields) {
          if (field.key.toLowerCase() == storageKey ||
  ...
  List<CreditCardFieldDraft> _fieldsForSection(_BankAccountSectionKind kind) {
    switch (kind) {
  ...
  void _removeField(_BankAccountSectionKind kind, CreditCardFieldDraft field) {
    setState(() {
      _isDirty = true;
  ...
  bool _hasMeaningfulContent(String title) {
    final hasSectionValue =
        <CreditCardFieldDraft>[..._primaryFields, ..._branchFields].any(
          (field) =>
  ...
  Future<void> _saveEdit() async {
    ...
    for (final field in <CreditCardFieldDraft>[
      ...
      if (!websiteAdded && normalizedStorageKey == 'website') {
        fields.add(
          EntryField(key: AppKdbxFieldKeys.url, value: value, isStandard: true),
        );
        websiteAdded = true;
      }
      fields.add(
        EntryField(key: storageKey, value: value, isProtected: isSecret),
      );
    }
                ...
                child: Column(
                  children: [
                    CreditCardSectionCard(
                      ...
                      onRemoveField: (field) =>
                          _removeField(_BankAccountSectionKind.primary, field),
                    ),
                    const SizedBox(height: 14),
                    CreditCardSectionCard(
                      ...
                      onRemoveField: (field) =>
                          _removeField(_BankAccountSectionKind.branch, field),
                    ),
                    const SizedBox(height: 14),
